...
When an Agent result has `status=requires_user_input` and a non-empty `clarifications` array, show every localized question, hint, option label, and description to the user, honor `single_select|text|file_select|image_select`, collect every answer, then continue once with repeated `--response` or `--answers-json`. File/image selection accepts an exact id, filename, or authorized absolute local path. Free-form media translation resolves subtitles versus dubbing and subtitle delivery without accepting model defaults. Custom `target_duration_sec` answers accept 10-600 seconds and common duration formats. The command preserves the original workflow, canonical media intent, task frame, context files, and compound execution plan. Runtime dubbing-reference quality choices resume the original run in place. Do not use it for install/authorization prompts that have no structured clarification.
